...
const Header = () => {
  ...
  const toggleMobileShop = () => setMobileShopOpen(!mobileShopOpen);
  const toggleMobileAbout = () => setMobileAboutOpen(!mobileAboutOpen);
    ...
    <header className="w-full">
      ...
      <div className="w-full bg-gray-100 py-4 px-4 md:px-12 flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="w-full flex justify-center md:hidden mb-4">
          <Link href={isLoggedIn && user?.role === "FARMER" ? "/landing_famer_store" : "/"}>
        ...
        <div className="hidden md:flex items-center md:w-auto md:flex-1">
          <Link href={isLoggedIn && user?.role === "FARMER" ? "/landing_famer_store" : "/"}>
            <div className="w-32 h-16 relative mr-4 flex-shrink-0">
              <Image 
                src="/farmerplace-logo.png" 
                alt="Fresh Harvest Logo" 
                fill
          ...
          <div className="relative w-full mx-4">
            <form onSubmit={handleSubmit} className="flex">
              <div className="relative w-24 md:w-32">
                ...
                >
                  <span className="truncate text-sm ml-2 font-semibold">
                    {searchType === "product" ? "Products" : "Farmers"}
                  </span>
          ...
          <div className="md:hidden">
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <div className="text-xs font-medium mt-1">Cart</div>
              </div>
            </Link>
          </div>
              ...
              <div className="relative">
                ...
                {accountDropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-md shadow-lg z-50 border border-gray-200">
                    {user?.role === "FARMER" ? (
                      <>
                        <Link href="/landing_famer_store">
                          <button onClick={() => setAccountDropdownOpen(false)} className="w-full text-left px-4 py-2 hover:bg-gray-100">My Store</button>
                        </Link>
                        <Link href="/farmer-orders">
                          <button onClick={() => setAccountDropdownOpen(false)} className="w-full text-left px-4 py-2 hover:bg-gray-100">My Orders</button>
                        </Link>
                      </>
                    ) : (
                      <>
                        <Link href="/account/profile">
                          <button onClick={() => setAccountDropdownOpen(false)} className="w-full text-left px-4 py-2 hover:bg-gray-100">Profile</button>
                        </Link>
          ...
          <ul className="flex flex-col py-2">
            {user?.role !== "FARMER" && (
              <li className="py-2 px-4 border-b border-gray-700">
                <Link href="/order-page" className="font-semibold hover:text-gray-300 transition-colors block">
                  CART
                </Link>
              </li>
            )}
