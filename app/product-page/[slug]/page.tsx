...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // 对URL中的编码字符进行解码，如把"sugar%20cane"解码为"sugar cane"
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        // Get all products and normalize categories
        stores.forEach((store: any) => {
          store.products.forEach((product: any) => {
            allProducts.push({
              ...
              price: product.price,
              category: product.category.toLowerCase().replace('_', ' '), // Convert SUGAR_CANE to "sugar cane"
              storeId: store.id,
        ...
        // Match slug with category
        let filtered = allProducts;
        if (slug !== "shop") {
          const normalizeForMatch = (str: string) =>
            str.toLowerCase()
               .replace(/[\s_-]+/g, "") // Remove spaces, underscores, and hyphens
               .replace(/ies$/, "y")    // Convert plurals to singular (e.g., veggies -> veggy)
               .replace(/s$/, "");      // Remove trailing s for other plurals

          const normalizedSlug = normalizeForMatch(decodedSlug);
          console.log("Normalized Slug:", normalizedSlug);
          
          filtered = allProducts.filter((product) => {
            const normalizedCategory = normalizeForMatch(product.category);
            console.log("Product Category:", product.category, "Normalized:", normalizedCategory);
            return normalizedCategory.includes(normalizedSlug) || 
        ...
        setCategoriesArray(
          Object.entries(categoryMap).map(([name, count]) => ({ 
            name: name.charAt(0).toUpperCase() + name.slice(1), // Capitalize category names
            count 
          }))
        );
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">
              {currentCategory === "All Products" 
                ? "All Products" 
                : `Products in ${decodeURIComponent(currentCategory).replace(/%20/g, " ")}`
              }
            </h2>
